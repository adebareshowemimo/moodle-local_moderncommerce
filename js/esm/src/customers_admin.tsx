...
/**
 * React admin customers list for Modern Commerce.
 *
 * @module     local_moderncommerce/customers_admin
...
export default function CustomersAdmin({
    ...
    useEffect(() => {
        ...
        void callMoodleService<CustomersResponse>(methodName, filters)
            .then((result) => {
                if (!cancelled) {
                    setData(result);
                }
    ...
    useEffect(() => {
        const refreshButton = document.getElementById("moderncommerce-customers-refresh");
    ...
    const changeSort = (sort: string) => {
        setFilters((current) => {
            ...
            return {
                ...current,
                sort,
                direction: sort === "customer" || sort === "email" ? "ASC" : "DESC",
                page: 0,
            };
        });
    };
    ...
    const renderCustomerActions = (customer: Customer) => (
        <div className={mcClasses("mc-table-design__actions justify-content-end")}>
            <McTableActionMenu
                label={`${labels.actions}: ${customer.fullname}`}
                items={[
                    {
                        key: "view",
        ...
        <section className={mcClasses("mc-product-admin")} aria-label={labels.title}>
                ...
                <div className={mcClasses("mc-stat-strip")} aria-label={labels.title}>
                    <article className={mcClasses("mc-stat-tile mc-stat-tile--primary")}>
                        ...
                        <div className={mcClasses("mc-stat-tile__body")}>
                            <span className={mcClasses("mc-stat-tile__label")}>{labels.totalcustomers}</span>
                    ...
                    <article className={mcClasses("mc-stat-tile mc-stat-tile--warning")}>
                        ...
                        <div className={mcClasses("mc-stat-tile__body")}>
                            ...
                            <strong className={mcClasses("mc-stat-tile__value")}>
                                {formatCount(stats.pendingorders)}
                            </strong>
                        </div>
                    ...
                    <article className={mcClasses("mc-stat-tile mc-stat-tile--info")}>
                        <i className="bi bi-bag-check mc-stat-tile__icon" aria-hidden="true" />
                        <div className={mcClasses("mc-stat-tile__body")}>
                        ...
                        <i className="bi bi-bag-check mc-stat-tile__watermark" aria-hidden="true" />
                    </article>
                </div>
            )}
                ...
                toolbar={(
                    <div className={mcClasses("mc-toolbar mc-product-toolbar mc-table-design-toolbar")}>
                        <div className={mcClasses("mc-product-toolbar__search")}>
                            <label className={mcClasses("mc-filter-label")} htmlFor="mc-customers-search">
                                {labels.search}
                            </label>
                            <input
                                className={mcClasses("mc-form-control")}
                                id="mc-customers-search"
            ...
            >
                <table className={mcClasses("table mc-table mc-product-table mb-0")} aria-label={labels.title}>
                            <thead>
                                <tr>
                                    <th scope="col">{renderSortButton("customer", labels.customer)}</th>
                                    <th scope="col">{renderSortButton("email", labels.email)}</th>
                                    <th scope="col" className="text-end">
                                        {renderSortButton("ordercount", labels.orders, "text-end")}
                                    </th>
                                    <th scope="col" className="text-end">
                                        {renderSortButton("totalspent", labels.totalspent, "text-end")}
                                    </th>
                                    <th scope="col" className="text-end">
                                        {renderSortButton("refundedorders", labels.refundedorders, "text-end")}
                                    </th>
                                    <th scope="col">{renderSortButton("lastorder", labels.lastorder)}</th>
                            ...
                            <tbody>
                                        ...
                                        <td colSpan={8}>
                                            <div className={mcClasses("mc-empty mc-empty--centered")}>
                                                ...
                                                <p className={mcClasses("mc-empty__title")}>
                                                    {total === 0 ? labels.nocustomers : labels.noresults}
                                                </p>
                                            </div>
                                        </td>
                                    </tr>
                                ...
                                {data?.items.map((customer) => (
                                    <tr key={customer.id}>
                                        <td>
                                            <a className="fw-semibold" href={customer.customerurl}>
                                                {customer.fullname}
                                            </a>
                                        </td>
                                        <td className={mcClasses("mc-cell-muted")}>{customer.email}</td>
                                        <td className="text-end">{formatCount(customer.ordercount)}</td>
                                        <td className="text-end fw-semibold">{customer.displaytotalspent}</td>
                                        <td className="text-end">{formatCount(customer.refundedorders)}</td>
